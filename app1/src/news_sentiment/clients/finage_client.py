...
"""
Finage Client
Client for Finage news and market data API
"""
...
class FinageClient(BaseNewsClient):
    """Client for Finage API"""
    
    def __init__(self, 
                 api_key: str,
                 base_url: str = "https://api.finage.co.uk",
                 rate_limiter: Optional[RateLimiter] = None):
        """Initialize Finage client"""
        super().__init__(
            api_key=api_key,
            base_url=base_url,
    ...
    async def fetch_news(self, 
        ...
        """Fetch news articles from Finage"""
        try:
            # Prepare parameters
            params = {
                'apikey': self.api_key,
            ...
            if 'data' in data:
                for article_data in data['data'][:limit]:
                    article = self._parse_article(article_data)
                    if article:
                        articles.append(article)
            
            logger.info(f"Fetched {len(articles)} news articles from Finage")
            return articles
            
        except Exception as e:
            logger.error(f"Error fetching Finage news: {e}")
            return []
    ...
    async def fetch_events(self,
        ...
        """Fetch economic events from Finage"""
        try:
            # Prepare parameters
            params = {
                'apikey': self.api_key,
            ...
            if 'data' in data:
                for event_data in data['data'][:limit]:
                    event = self._parse_event(event_data)
                    if event:
                        events.append(event)
            
            logger.info(f"Fetched {len(events)} economic events from Finage")
            return events
            
        except Exception as e:
            logger.error(f"Error fetching Finage events: {e}")
            return []
    ...
    def _parse_article(self, article_data: Dict[str, Any]) -> Optional[NewsArticle]:
        ...
        try:
            ...
            source_name = article_data.get('source', 'Finage')
            ...
            if not article_id:
                ...
                article_id = f"finage_{content_hash}"
            ...
            logger.error(f"Error parsing Finage article: {e}")
            return None
    ...
    def _parse_event(self, event_data: Dict[str, Any]) -> Optional[EconomicEvent]:
        ...
        try:
            ...
            logger.error(f"Error parsing Finage event: {e}")
            return None
